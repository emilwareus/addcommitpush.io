...
| `GET`, `PUT`           | `/v1/contradictions/{id}`          | Inspect, confirm, dismiss, or resolve a conflict               |
...
| `GET`    | `/v1/oauth/{provider}/callback`         | Validate state, connect, then redirect to the fixed frontend settings URL |
...
OAuth callbacks return `303 See Other` to the configured `LIFE_FRONTEND_BASE_URL`
connector settings page. Redirect query parameters contain only the provider and
a stable success or error code; OAuth codes, state, tokens, connectors, and raw
provider messages are never included.
