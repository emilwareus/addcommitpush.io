...
func TestAgentLoopWithOptions(t *testing.T) {
	...
	orch := NewAgentLoop(
		bus,
		cfg,
		WithLoopClient(mockClient),
		WithLoopTools(mockTools),
	)
...
func TestAgentLoopResearchFullWorkflow(t *testing.T) {
	bus := events.NewBus(100)
	cfg := &config.Config{
		OpenRouterAPIKey: "test-key",
		BraveAPIKey:      "test-brave-key",
...
func TestAgentLoopEventEmission(t *testing.T) {
	bus := events.NewBus(100)
	cfg := &config.Config{
		OpenRouterAPIKey: "test-key",
		BraveAPIKey:      "test-brave-key",
...
func TestLoopResultFields(t *testing.T) {
	result := &LoopResult{
		Query:         "test query",
...
func TestAgentLoopNilBusCreation(t *testing.T) {
	cfg := &config.Config{
		OpenRouterAPIKey: "test-key",
		BraveAPIKey:      "test-brave-key",
		Model:            "test-model",
