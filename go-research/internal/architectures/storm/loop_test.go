...
// TestStormLoopCreation tests that the orchestrator can be created
func TestStormLoopCreation(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(100)
	defer bus.Close()

	orch := NewStormLoop(bus, cfg)
...
// TestStormLoopWithOptions tests option injection
func TestStormLoopWithOptions(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(100)
	defer bus.Close()
...
// TestStormLoopFullWorkflow tests the complete STORM workflow
func TestStormLoopFullWorkflow(t *testing.T) {
	cfg := stormTestConfig()
...
// TestStormLoopParallelConversations tests that conversations run in parallel
func TestStormLoopParallelConversations(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(200)
	defer bus.Close()
...
// TestStormLoopEmitsEvents tests that all expected events are emitted
func TestStormLoopEmitsEvents(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(200)
	defer bus.Close()
...
// TestStormLoopContextCancellation tests cancellation handling
func TestStormLoopContextCancellation(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(100)
	defer bus.Close()
...
// TestStormLoopTwoPhaseOutline tests that outline is generated in two phases
func TestStormLoopTwoPhaseOutline(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(200)
	defer bus.Close()
...
// TestStormLoopConversationDataInResult tests that conversation data is in result
func TestStormLoopConversationDataInResult(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(200)
	defer bus.Close()
...
// TestStormLoopAnalysisUsesConversationFacts tests that analysis uses facts from conversations
func TestStormLoopAnalysisUsesConversationFacts(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(200)
	defer bus.Close()
...
// TestStormLoopPerspectiveWithBasicFactWriter tests that Basic Fact Writer is included
func TestStormLoopPerspectiveWithBasicFactWriter(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(200)
	defer bus.Close()
...
// TestStormLoopCostAccumulation tests that costs are accumulated from all phases
func TestStormLoopCostAccumulation(t *testing.T) {
	cfg := stormTestConfig()
	bus := events.NewBus(200)
	defer bus.Close()
