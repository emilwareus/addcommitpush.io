import { z } from 'zod';
import { updateConnectorRequestSchema, uuidSchema } from '@/lib/life/contracts';
import { revokeConnector, updateConnector } from '@/lib/life/queries.server';
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    const body = await readLifeMutation(request, updateConnectorRequestSchema);
    const id = uuidSchema.parse((await context.params).id);
    return privateJson(await updateConnector(id, body));
  } catch (error) {
