...
activity, health measurements, researched claims, and encrypted connector
credentials. PostgreSQL contains searchable plaintext memory. Connector
credentials are encrypted with AES-256-GCM and a key kept outside PostgreSQL.
...
| Database dump exposes connector secrets | AES-256-GCM, random nonces, associated data, and a separate runtime key                                       | Runtime compromise can access ciphertext and key                  |
...
| Replayed webhooks                       | Timestamped HMAC verification on Linear webhooks                                                              | A compromised webhook secret can enqueue sync jobs                |
...
Connectors import bounded surfaces: GitHub activity, Linear issues, Slack
messages, and email metadata/snippets. Email does not import attachment bytes or
full message bodies. Online research never runs in the background.

## Incident and deletion semantics

Revoking a connector removes local encrypted credentials but does not invalidate
the upstream API key or token at the provider. Deleting an owner cascades through live PostgreSQL rows. Backups,
...
owner's audit history. If the encryption key leaks, rotate it, redeploy, and
re-connect affected connectors with new credentials.
