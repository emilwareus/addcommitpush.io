...
async function fetchGitHubDataInternal(): Promise<GitHubStatusData> {
  ...
  accountsData.forEach((account) => {
    if (account.commits) {
      account.commits.data.user.contributionsCollection.contributionCalendar.weeks
        ...
        .forEach((day) => {
          commitsByDayMap.set(day.date, (commitsByDayMap.get(day.date) || 0) + day.contributionCount)
        })
    }
  })
  ...
  if (referenceAccount) {
    allDates = referenceAccount
      .commits!.data.user.contributionsCollection.contributionCalendar.weeks
      .flatMap((week) => week.contributionDays)
      .map((day) => day.date)
  } else {
    const dates: string[] = []
