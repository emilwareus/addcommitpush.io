  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full max-w-5xl mx-auto px-8">
      ...
      <motion.div
        ...
        style={{
          boxShadow:
            '0 0 20px oklch(0.7 0.14 195 / 0.2), inset 0 0 20px oklch(0.7 0.14 195 / 0.05)',
        }}
      >
        <p className="text-base text-muted-foreground mb-1">
          Google Diffusion vs OpenAI Deep Research (DeepConsult)
        </p>
      ...
      >
        ...
        <p className="text-sm text-muted-foreground/70 mb-3 text-center max-w-2xl mx-auto">
          100 PhD-level research tasks across 22 fields, scored by 70+ expert annotators. RACE
          evaluates Comprehensiveness, Insight, Instruction Following, Readability.
        </p>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-card/80 border-b border-border/40">
                ...
                <th className="px-4 py-2.5 text-sm font-semibold text-muted-foreground text-right">
                  RACE
                </th>
                <th className="px-4 py-2.5 text-sm font-semibold text-muted-foreground text-right">
                  License
              ...
              {leaderboard.map((entry, i) => (
                <motion.tr
                  ...
                  className={`border-b border-border/20 ${
                    entry.highlight ? 'bg-primary/10 border-l-2 border-l-primary' : 'bg-card/40'
                  }`}
                >
                  <td className="px-4 py-3 text-lg font-mono text-muted-foreground">
                    {entry.rank}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`text-lg font-semibold ${entry.highlight ? 'text-primary' : 'text-foreground'}`}
                    >
                      {entry.name}
                    </span>
                  ...
                  <td
                    className={`px-4 py-3 text-xl font-bold text-right ${
